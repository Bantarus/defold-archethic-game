...
---@param complete_function fun(self:object, message_id:hash, message:{play_id:number}, sender:url)|nil function to call when the sound has finished playing or stopped manually via sound.stop.
...
---
---Stop playing all active voices or just one voice if <code>play_id</code> provided
---@param url string|hash|url the sound component that should stop
---@param stop_properties {play_id:number}|nil optional table with properties
...
--- <code>play_id</code> should be specified in case you want to receive <code>sound_done</code> or <code>sound_stopped</code> in <code>on_message()</code>.
...
---This message is sent back to the sender of a <code>play_sound</code> message
